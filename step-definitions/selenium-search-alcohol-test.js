...
module.exports = function () {
    ...
    this.Then(/^I should be given a list of products that matches "([^"]*)"$/, async function (searchString) {
        await helpers.getElementsContainingText(searchString);
    ...
    /* this.Given(/^that I am searching for (\d+)$/, async function (searchNumber) {
     ...
     this.Then(/^I should be given a list of products that matches (\d+)$/, function (searchResult) {
         assert.notEqual(searchResult, null, "Could not find the search result");
     });*/
}